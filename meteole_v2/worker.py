...
class MeteoleTask(QgsTask):
        ...
        if horizon_col and df[horizon_col].nunique() > 1:
            for h_val, sub in df.groupby(horizon_col):
                ...
                info  = prepare_layer_files(sub.reset_index(drop=True),
                                            f"{label[:40]} — {h_str}", indicator,
                                            clip_bbox=clip_bbox,
                                            clip_cutline_path=clip_cutline)
                info["horizon_label"] = h_str
                info["indicator"]     = indicator
                layer_files.append(info)
        else:
            info = prepare_layer_files(df, label[:50], indicator,
                                       clip_bbox=clip_bbox,
                                       clip_cutline_path=clip_cutline)
            info["horizon_label"] = ""
            info["indicator"]     = indicator
    ...
    def _get_vigilance(self):
        ...
        if self.kwargs.get("load_phenomenon") or self.kwargs.get("load_timelaps"):
            ...
            # Détail PAR ALÉA et PAR DÉPARTEMENT (vent, orages, pluie-inondation…)
            try:
                from .vigilance_utils import parse_hazards
                result["hazards"] = parse_hazards(vigi.get_map())
            except Exception as e:
                result["hazards_error"] = str(e)
        ...
        for time_step in time_horizons:
            ...
            for var_name, df in var_frames:
                ...
                if selected_vars and var_name not in selected_vars:
                    continue
                from .arome_om import grib_var_label_clean
                # Label français SANS unité : prepare_layer_files ajoutera
                # l'unité (éventuellement convertie : K→°C, etc.)
                var_label = grib_var_label_clean(var_name)
                ...
                try:
                    info = prepare_layer_files(
                        df, display_label,
                        var_name,  # indicator_name = shortName pour style/unité
                        clip_bbox=clip_bbox,
                        clip_cutline_path=clip_cutline)
                    info["horizon_label"] = time_step
                    info["indicator"]     = var_name
                    # NB : on conserve le layer_name renvoyé par
                    # prepare_layer_files, qui inclut l'unité convertie.
                    layer_files.append(info)
                except Exception as e:
