...
except Exception:  # pragma: no cover - environnement sans qgis.gui
...
class MeteoleDialog(QDialog):
    ...
    def _build_page_variable(self):
        ...
        # Bascule : afficher toutes les variables (désactive le filtre)
        self.chk_show_all_vars = QCheckBox(
            "Afficher toutes les variables (avancé : niveaux de pression, flux…)")
        self.chk_show_all_vars.setChecked(False)
        self.chk_show_all_vars.setStyleSheet("font-size:11px;color:#888;")
        self.chk_show_all_vars.toggled.connect(self._on_show_all_toggled)
        vl.addWidget(self.chk_show_all_vars)
    ...
    def _build_page_options(self):
        ...
        # --- Emprise d'une couche vectorielle ---
        if QgsMapLayerComboBox is not None:
            vec_row = QHBoxLayout()
            vec_row.addWidget(QLabel("Couche vecteur :"))
            self.cb_extent_layer = QgsMapLayerComboBox()
            self.cb_extent_layer.setFilters(QgsMapLayerProxyModel.VectorLayer)
            vec_row.addWidget(self.cb_extent_layer, 1)
            geo_vl.addLayout(vec_row)
            btn_vec = QPushButton("🗺 Utiliser l'emprise de la couche vectorielle")
            btn_vec.clicked.connect(self._use_vector_layer_extent)
            geo_vl.addWidget(btn_vec)
            self.chk_clip_geom = QCheckBox(
                "Découper le raster sur la géométrie de la couche "
                "(et non l'emprise rectangulaire)")
            self.chk_clip_geom.setChecked(True)
            self.chk_clip_geom.setStyleSheet("font-size:11px;color:#555;")
            geo_vl.addWidget(self.chk_clip_geom)
        else:
            self.cb_extent_layer = None
            self.chk_clip_geom = None
    ...
    def _current_clip_bbox(self):
        """
        Emprise courante (lon_min, lon_max, lat_min, lat_max) en WGS84,
        ou None si « Territoire entier » est coché. Utilisée pour recadrer
        le raster (emprise du canevas ou d'une couche vectorielle).
        """
        if self.chk_full_territory.isChecked():
            return None
        try:
            return (float(self.sp_lon_min.text() or 0),
                    float(self.sp_lon_max.text() or 0),
                    float(self.sp_lat_min.text() or 0),
                    float(self.sp_lat_max.text() or 0))
        except ValueError:
            return None
    ...
    def _use_canvas_extent(self):
        ...
        # Emprise du canevas = rectangulaire : pas de découpe géométrique
        self._clip_layer_id = None

    def _use_vector_layer_extent(self):
        """Renseigne la bbox depuis l'emprise d'une couche vectorielle (WGS84)."""
        if getattr(self, "cb_extent_layer", None) is None:
            return
        layer = self.cb_extent_layer.currentLayer()
        if layer is None:
            QMessageBox.information(
                self, "Emprise vectorielle",
                "Aucune couche vectorielle sélectionnée.\n"
                "Ajoutez une couche vecteur au projet, puis réessayez.")
            return
        ...
        self.sp_lon_min.setText(str(extent.xMinimum()))
        self.sp_lon_max.setText(str(extent.xMaximum()))
        self.sp_lat_min.setText(str(extent.yMinimum()))
        self.sp_lat_max.setText(str(extent.yMaximum()))
        self.chk_full_territory.setChecked(False)
        # Mémorise la couche pour la découpe géométrique (cutline) au chargement
        self._clip_layer_id = layer.id()
        geom_note = ("Le raster sera découpé sur la géométrie de la couche."
                     if (self.chk_clip_geom and self.chk_clip_geom.isChecked())
                     else "Le raster sera recadré sur l'emprise rectangulaire.")
        self._log(
            f"[INFO] Emprise de la couche « {layer.name()} » appliquée (WGS84) : "
            f"lon [{extent.xMinimum():.3f}, {extent.xMaximum():.3f}] "
            f"lat [{extent.yMinimum():.3f}, {extent.yMaximum():.3f}]. {geom_note}")

    def _current_cutline_path(self):
        """
        Chemin d'un GeoJSON (WGS84) décrivant la géométrie de la couche de
        découpe, ou None si aucune découpe géométrique n'est demandée.
        Utilisé comme cutline gdal.Warp pour découper le raster sur les
        contours réels de la couche vectorielle.
        """
        if self.chk_full_territory.isChecked():
            return None
        if not getattr(self, "chk_clip_geom", None) or not self.chk_clip_geom.isChecked():
            return None
        lid = getattr(self, "_clip_layer_id", None)
        if not lid:
            return None
        ...
        if layer is None:
            return None
        try:
            return self._build_cutline_for_layer(layer)
        except Exception as e:
            self._log(f"[WARN] Découpe géométrique impossible ({e}). "
                      f"Recadrage rectangulaire utilisé.")
            return None

    def _build_cutline_for_layer(self, layer):
        """
        Exporte la géométrie de la couche en GeoJSON WGS84 temporaire,
        reprojetée si nécessaire. Retourne le chemin du fichier, ou None
        si la couche ne contient aucune géométrie exploitable.
        """
        import json
        import tempfile
        ...
        for feat in layer.getFeatures():
            ...
            if xform is not None:
                geom = QgsGeometry(geom)   # copie pour ne pas altérer la couche
                geom.transform(xform)
            try:
                gj_geom = json.loads(geom.asJson())
            except Exception:
                continue
        ...
        path = os.path.join(tempfile.mkdtemp(), "cutline.geojson")
    ...
    def _get_forecast_params(self):
        ...
        # L'indicateur BRUT (nom WCS) est stocké en donnée de l'item ;
        # le texte affiché est le libellé français.
        raw_ind  = self.cb_indicator.currentData()
        disp_ind = self.cb_indicator.currentText().strip()
        indicator = (raw_ind or disp_ind) or None

        params = {
            "model":             model_key,
            "indicator":         indicator,
            "indicator_label":   disp_ind or indicator,
            "territory":         territory,
            "run":               None,
    ...
    def _populate_metro_indicators(self):
        """
        (Re)peuple la liste des indicateurs métropole : applique le filtre des
        variables significatives (sauf si « Afficher toutes » est coché),
        traduit en français, et conserve le nom brut en donnée d'item.
        Retourne le nombre d'indicateurs affichés.
        """
        from .meteo_vars import filter_metro_indicators, translate_indicator_fr
        raw = list(getattr(self, "_all_indicators", []))
    ...
    def _on_show_all_toggled(self, _):
        """Re-filtre les listes de variables (métropole + OM)."""
        if getattr(self, "_all_indicators", None):
    ...
    def _on_load(self):
        ...
        if is_om:
            ...
            task = MeteoleWorker(
                ...
                clip_bbox=om_params.get("clip_bbox"),
                clip_cutline_path=om_params.get("clip_cutline_path"),
            )
        else:
            # Mode WCS métropole
    ...
    def _on_om_grid_changed(self, _):
        ...
        show_all = (self.chk_show_all_vars.isChecked()
                    if getattr(self, "chk_show_all_vars", None) else False)
        entries = package_variable_entries(packages, significant_only=not show_all)
        for e in entries:
    ...
    def _on_om_capabilities_done(self, result):
        ...
        show_all = (self.chk_show_all_vars.isChecked()
                    if getattr(self, "chk_show_all_vars", None) else False)
        entries = package_variable_entries(all_packages, significant_only=not show_all)
        for e in entries:
            self.cb_om_package.addItem(e["display"], e)
    ...
    def _on_vigilance_done(self, result):
        self._set_busy(False)
        from .vigilance_utils import (national_summary, hazards_by_department,
                                       summarize_department)

        COLOR_BG = {1: ("#00aa00", "white"), 2: ("#ffdd00", "black"),
                    3: ("#ff8800", "white"), 4: ("#dd0000", "white")}
        # Étiquettes meteole (anglais) → français, pour le repli df_phen
        PHENOM_FR = {
            "wind": "Vent violent", "rain": "Pluie-inondation",
            "flood": "Inondation", "snow_ice": "Neige-verglas",
            "storm": "Orages", "heat_wave": "Canicule",
            "extreme_cold": "Grand froid", "avalanches": "Avalanches",
            "waves_submergence": "Vagues-submersion",
        }
        COLOR      = COLOR_BG
        ...
        if hazards:
            # --- Synthèse nationale PAR ALÉA ---
            rows = national_summary(hazards, "J")
            html = ("<table border='1' cellpadding='3' cellspacing='0' "
                    "style='border-collapse:collapse;font-size:11px;'>")
            html += ("<tr style='background:#dde;'><th>Aléa</th>"
                     "<th>Niveau max</th><th>Départements ≥ jaune</th></tr>")
            if not rows:
                html += ("<tr><td colspan='3' style='text-align:center;'>"
                         "Aucune vigilance particulière (vert partout).</td></tr>")
            for r in rows:
                bg, fg = COLOR_BG.get(r["color_id_max"], ("#fff", "black"))
                html += (f"<tr><td>{r['phenomenon_fr']}</td>"
                         f"<td style='background:{bg};color:{fg};text-align:center;'>"
                         f"<b>{r['color_fr']}</b></td>"
                         f"<td style='text-align:center;'>{r['nb_departements']}</td></tr>")
            html += "</table>"

            # --- Départements en vigilance orange/rouge + aléas associés ---
            hbd = hazards_by_department(hazards, "J")
            notable = []
            for code, hz in hbd.items():
                mx = max(hz.values()) if hz else 1
                if mx >= 3:
                    notable.append((code, mx, summarize_department(hz, min_color=2)))
            notable.sort(key=lambda x: (-x[1], x[0]))
            if notable:
                html += ("<br><b style='font-size:11px;'>Départements en "
                         "orange / rouge :</b>")
                html += ("<table border='1' cellpadding='3' cellspacing='0' "
                         "style='border-collapse:collapse;font-size:11px;'>")
                html += ("<tr style='background:#dde;'><th>Dépt</th>"
                         "<th>Aléa(s)</th></tr>")
                for code, mx, txt in notable:
                    bg, fg = COLOR_BG.get(mx, ("#fff", "black"))
                    html += (f"<tr><td style='background:{bg};color:{fg};"
                             f"text-align:center;'><b>{code}</b></td>"
                             f"<td>{txt or '—'}</td></tr>")
                html += "</table>"

            self.tbl_vigilance.setHtml(html)
            self._log(f"[OK] Vigilance chargée — {len(hbd)} départements, "
                      f"{len(rows)} aléa(s) actif(s).")
        else:
            # --- Repli : ancien tableau national basé sur df_phen ---
            ...
            if df_phen is not None:
                ...
                self._log("[OK] Tableau de vigilance chargé "
                          "(détail par aléa indisponible).")

        # --- Couche départements (colorée + détail par aléa si disponible) ---
        df_time = result.get("timelaps")
        hbd_layer = hazards_by_department(hazards, "J") if hazards else None
        has_time  = (df_time is not None
                     and getattr(df_time, "columns", None) is not None
                     and "max_color_id" in df_time.columns)
        if hbd_layer or has_time:
            try:
                lyr = add_vigilance_dept_layer(df_time, result.get("phenomenon"),
                                               hazards_by_dept=hbd_layer)
                if lyr and lyr.isValid():
                    QgsProject.instance().addMapLayer(lyr)
                    msg = "[OK] Couche vigilance par département ajoutée"
                    msg += (" (avec détail par aléa : survolez ou ouvrez la "
                            "table attributaire)." if hbd_layer else ".")
                    self._log(msg)
            except Exception as e:
