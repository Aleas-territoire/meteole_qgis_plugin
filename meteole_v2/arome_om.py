...
def package_variable_entries(packages: list[str],
                             significant_only: bool = True) -> list[dict]:
    """
    ...
    significant_only : si True (défaut), ne conserve que les variables de surface
        les plus communes (cf. meteo_vars.OM_KEEP_SHORTNAMES) et masque les
        paquets isobares/hybrides (IP*/HP*), réservés à un usage avancé.
    """
    from .meteo_vars import is_significant_om, display_unit
    ...
    for pkg in packages:
        ...
        # En mode « significatif », on masque les paquets multi-niveaux
        # (variables isobares/hybrides), peu parlants pour un public large.
        if significant_only and is_multilevel:
            continue
            ...
            if is_multilevel:
                ...
                for v in vars_in_pkg:
                    if v in seen_surface_vars:
                        continue
                    # Mode significatif : ne garder que les variables communes
                    if significant_only and not is_significant_om(v):
                        continue
                    seen_surface_vars.add(v)
                    # Libellé = nom français + unité AFFICHÉE (convertie) pour
                    # rester cohérent avec la couche chargée (ex. °C, hPa, km/h).
                    clean = grib_var_label_clean(v)
                    du    = display_unit(v)
                    label = f"{clean} ({du})" if du else clean
                    display = f"{label}  [{pkg}]" if clean != v else f"{v}  [{pkg}]"
                    entries.append({
                        "display": display,
                        "package": pkg,
                        "var_name": v,
...
def grib_var_label(short_name: str) -> str:
    """Retourne le label français d'une variable GRIB2 (avec unité), ou le shortName brut."""
...
def grib_var_label_clean(short_name: str) -> str:
    """
    Retourne le label français SANS unité d'une variable GRIB2.
    L'unité (potentiellement convertie) est ajoutée plus tard par
    layer_utils.prepare_layer_files, pour éviter toute incohérence
    (ex. afficher « (K) » alors que les valeurs sont en °C).
    """
    info = GRIB_VAR_INFO.get(short_name)
    return info[0] if info else short_name
